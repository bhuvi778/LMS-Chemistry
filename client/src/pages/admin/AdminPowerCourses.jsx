...
export default function AdminPowerCourses() {
  ...
  const load = () => {
    setLoading(true);
    return api
      .get('/courses?includeUnpublished=true&isPowerCourse=true')
  ...
  const filtered = useMemo(() => {
    return list.filter((c) => {
      ...
      if (q && !(`${c.title} ${c.instructor} ${c.category} ${c.powerCourseType}`.toLowerCase().includes(q.toLowerCase())))
        return false;
      return true;
    });
  }, [list, q, cat]);
  ...
  const getPowerTypeBadgeColor = (type) => {
    switch (type) {
      case 'micro':
              ...
              <div key={c._id} className="card overflow-hidden flex flex-col group">
                <div className="relative h-40 overflow-hidden bg-slate-100">
                  ...
                  <div className="absolute top-2 left-2 flex flex-col gap-1">
                    <span className="chip bg-white/95 backdrop-blur text-brand-700 shadow text-[10px]">
                      {(c.categories?.[0] || c.category) || 'No Category'}
                    </span>
                    <span className={`px-2 py-0.5 rounded text-[9px] uppercase font-bold text-center tracking-wider shadow ${getPowerTypeBadgeColor(c.powerCourseType)}`}>
                      {c.powerCourseType || 'other'}
                    </span>
                  </div>
                  <button
                  ...
                  <div className="absolute bottom-2 right-2 px-2 py-0.5 rounded bg-black/60 backdrop-blur text-white font-semibold text-[10px] flex items-center gap-1">
                    <Calendar size={10} />
                    {c.powerCourseDuration || 7} Days
                  </div>
                </div>
            ...
            <tbody>
              {paged.map((c) => (
                <tr key={c._id} className="border-t border-slate-100">
                  ...
                  <td className="p-4">
                    <span className={`chip ${getPowerTypeBadgeColor(c.powerCourseType)} text-[10px] uppercase font-bold`}>
                      {c.powerCourseType}
                    </span>
                  </td>
                  <td className="p-4 font-semibold">{c.powerCourseDuration || 7} Days</td>
                  <td className="p-4">
